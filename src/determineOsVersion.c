...
//			if (line[0] == 'v') {
//				strcpy((*fingerprint).osVersion, line + 1);
////				printf("version is %s",(*fingerprint).osVersion);
//			} else
			if (strlen(line) >= 43 && strlen(line) < 47 && line[41] == ' ' && line[32] == ' ') {
...
//is match success
int matchByIndex(int osNumber, unsigned char md5digest[16], int offset, int availableOs[],
		int matchCounts[]) {
	//search hash table
	int i;
	int matchCount = 0;
...
//is match success
int match(int osNumber, unsigned char md5digest[16], int *md5Index, int availableOs[]) {
	//search hash table
	int i;
	int matchCount = 0;
	int matchIndex = -1;
	...
	for (i = 0; i < osNumber; i++) {
		...
		if (isMd5ArrayEmpty(fingerprints[i].md5[*md5Index]) == 0) {
			...
			gapNo++;
			continue;
		} else if (memcmp(md5digest, fingerprints[i].md5[*md5Index], 16) == 0) {
			matchIndex = i;
			matchCount++;
		} else {
			//kick out a os signature
...
//:w		printf("%x %x\n", clusters[i].start, clusters[i].end);
		...
		for (; vAddr < clusters[i].end; vAddr += 0x1000) {
			...
			if (vAddr == out_pc)
				code_init(mem, vAddr, pageSize, dsmPages, virtualAddrs, 1, calledPages,
						&codePageNo);
			else
				code_init(mem, vAddr, pageSize, dsmPages, virtualAddrs, 0, calledPages,
						&codePageNo);
		}
...
//		printf("start:%x, end:%x: len:%x kernel\n", ranges[i].start, ranges[i].end, ranges[i].len);
	...
	for (; startVirtualAddr <= ranges[max_index].end; startVirtualAddr += 0x1000) {
		...
		if (dsmPages[pageIndex] == 1) {
			int offset = (startVirtualAddr - ranges[max_index].start) / 4096;
